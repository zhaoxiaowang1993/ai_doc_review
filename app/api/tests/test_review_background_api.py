...
class TestReviewBackgroundAPI(unittest.TestCase):
    def test_start_status_cancel(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                app.dependency_overrides[get_issues_service] = lambda: issues_service

                client = TestClient(app)

                started = client.post(f"/api/v1/review/{doc_id}/start").json()
                self.assertIn(started["status"], ("running", "completed"))
                run_id = started.get("run_id")
                self.assertTrue(run_id)

                status = client.get(f"/api/v1/review/{doc_id}/status").json()
                self.assertIn(status["status"], ("running", "completed", "cancelled"))

                cancelled = client.post(f"/api/v1/review/{doc_id}/cancel").json()
                self.assertIn(cancelled["status"], ("cancel_requested", "cancelled", "completed"))
            finally:
